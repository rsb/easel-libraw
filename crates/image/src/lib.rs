...
pub struct ResultBuffer {
  inner: Arc<RwLock<Option<ImageBuffer>>>,
}
...
impl ResultBuffer {
  ...
  pub fn with_image(buffer: ImageBuffer) -> Self {
    Self {
      inner: Arc::new(RwLock::new(Some(buffer))),
    }
  }

  pub fn read<T>(&self, f: impl FnOnce(&ImageBuffer) -> T) -> Result<Option<T>, fail::Error> {
    let guard = self.inner.read().map_err(|_| {
      fail::Error::resource("result buffer rwlock poisoned").context("ResultBuffer::read failed")
    })?;
    Ok(guard.as_ref().map(f))
  }

  pub fn write(&self, buffer: ImageBuffer) -> Result<(), fail::Error> {
    let mut guard = self.inner.write().map_err(|_| {
      fail::Error::resource("result buffer rwlock poisoned").context("ResultBuffer::write failed")
    })?;
    *guard = Some(buffer);
    Ok(())
  }
