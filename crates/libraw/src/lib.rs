...
use lab_fail::{self as fail, ResultExt};
...
impl LibRawProcessor {
  ...
  fn open_file(&mut self, c_path: &CString) -> Result<(), fail::Error> {
    let rc = unsafe { sys::libraw_open_file(self.ptr, c_path.as_ptr()) };
    if rc != 0 {
  ...
  fn unpack(&mut self) -> Result<(), fail::Error> {
    let rc = unsafe { sys::libraw_unpack(self.ptr) };
    if rc != 0 {
  ...
  fn unpack_thumb(&mut self) -> Result<(), fail::Error> {
    let rc = unsafe { sys::libraw_unpack_thumb(self.ptr) };
    if rc != 0 {
  ...
  fn dcraw_process(&mut self) -> Result<(), fail::Error> {
    let rc = unsafe { sys::libraw_dcraw_process(self.ptr) };
    if rc != 0 {
  ...
  fn dcraw_make_mem_thumb(&mut self) -> Result<ImageBuffer, fail::Error> {
    ...
    let thumb_ptr = unsafe { sys::libraw_dcraw_make_mem_thumb(self.ptr, &mut errcode) };
    ...
    let result = unsafe {
      ...
      sys::libraw_dcraw_clear_mem(thumb_ptr);
      (owned, format, width, height, bits, colors)
    };

    let (owned, format, width, height, bits, colors) = result;

    if format == sys::LibRaw_image_formats_LIBRAW_IMAGE_JPEG {
      decode_jpeg_thumbnail(&owned)
    } else if format == sys::LibRaw_image_formats_LIBRAW_IMAGE_BITMAP {
      if bits == 8 && colors == 3 {
  ...
  fn dcraw_make_mem_image(&mut self) -> Result<ImageBuffer, fail::Error> {
    ...
    let image_ptr = unsafe { sys::libraw_dcraw_make_mem_image(self.ptr, &mut errcode) };
    ...
    let (owned_rgb, width, height) = unsafe {
      ...
      if image.type_ != sys::LibRaw_image_formats_LIBRAW_IMAGE_BITMAP {
        sys::libraw_dcraw_clear_mem(image_ptr);
...
fn libraw_error(rc: i32, func: &str) -> String {
  let ptr = unsafe { sys::libraw_strerror(rc) };
  if ptr.is_null() {
